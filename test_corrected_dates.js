// Test corrected date calculation logic
function testCorrectedDateCalculation() {
    const today = new Date('2026-01-01'); // Using the current date from environment
    const months = 3;

    console.log('Testing CORRECTED FNB date calculation logic');
    ...
    for (let i = 0; i < months; i++) {
        ...
        // Calculate end date first (current date minus i*30 days)
        endDate.setDate(today.getDate() - i * daysPerPeriod);

        // Calculate start date (end date minus 30 days)
        startDate.setDate(endDate.getDate() - daysPerPeriod);
