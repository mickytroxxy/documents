...
export const generateIdImage = async (idInfo: IdInfo, outputPath: string, photoBuffer?: Buffer, signatureBuffer?: Buffer) => {
    ...
    // Use provided photo or fallback to default
    ...
    // Create SVG elements with dynamic data
  ...
  <svg width="1200" height="100">
    <style>
      .title { fill: ${CONFIG.backColor}; font-size: ${fontSize}px; font-weight: bold; font-family: Arial; }
    </style>
    ...
    try {
        ...
        const hologramSvg = `
            <svg width="${width}" height="${height}">
                ...
                <line x1="0" y1="${height * 0.3}" x2="${width}" y2="${height * 0.3}" stroke="#00ffff" stroke-width="2" opacity="0.2"/>
                <line x1="0" y1="${height * 0.5}" x2="${width}" y2="${height * 0.5}" stroke="#ff00ff" stroke-width="1.5" opacity="0.15"/>
                <line x1="0" y1="${height * 0.7}" x2="${width}" y2="${height * 0.7}" stroke="#ffff00" stroke-width="2" opacity="0.2"/>
            </svg>
        `;
            ...
            <body>
                <div class="container">
                    <div class="id-container">
                        <img class="front" src="${frontBase64}" alt="Front ID" width="500"/>
                    </div>
                    <div class="id-container">
...
export const createIdHandler = async (req: Request, res: Response) => {
    try {
        ...
        // Home Affairs mode: only ID number required, feature not ready yet
        ...
        // Get photo from multipart form data
        let photoBuffer: Buffer | undefined;
        if (req.files && req.files.photo) {
            const photoFile = req.files.photo as any;
        ...
        // Create output folder with ID number
        const folder = path.join(process.cwd(), 'files', id);
        ...
        return res.status(200).json({
            ...
            data: {
                frontImage: `${baseUrl}/${id}/${path.basename(frontPath)}`,
                backImage: `${baseUrl}/${id}/${path.basename(backPath)}`,
                frontEnhancedImage: `${baseUrl}/${id}/${path.basename(frontEnhancedPath)}`,
                backEnhancedImage: `${baseUrl}/${id}/${path.basename(backEnhancedPath)}`,
                frontPdf: `${baseUrl}/${id}/${path.basename(frontPdfPath)}`,
                backPdf: `${baseUrl}/${id}/${path.basename(backPdfPath)}`,
                combinedPdf: `${baseUrl}/${id}/${path.basename(combinedPdfPath)}`,
                idInfo,
                requiredCost
            }
        });
